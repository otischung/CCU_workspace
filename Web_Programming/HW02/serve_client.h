...
static int game_act(struct game *game, uint32_t x, uint32_t y);
...
static int game_act(struct game *game, uint32_t x, uint32_t y) {
    ...
    game->martix[x][y] = game->players[game->turn];
    game->turn = !game->turn;
    return 0;
}
...
static int game_result(struct game *game) {
    for (int i = 0; i < 3; ++i) {
        if (game->martix[i][0] == game->martix[i][1] && game->martix[i][1] == game->martix[i][2]) {
            return game->martix[i][0];
        }
        if (game->martix[0][i] == game->martix[1][i] && game->martix[1][i] == game->martix[2][i]) {
            return game->martix[0][i];
        }
    }
    if (game->martix[0][0] == game->martix[1][1] && game->martix[1][1] == game->martix[2][2]) {
        return game->martix[0][0];
    }
    if (game->martix[0][2] == game->martix[1][1] && game->martix[1][1] == game->martix[2][0]) {
        return game->martix[0][2];
    }
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            if (game->martix[i][j] == -1) return -1;
        }
    }
    return -2;
}
...
static inline int login(int fd) {
    ...
    if (read_n_and_string(fd, username, MAXLINE) != -1 &&
        read_n_and_string(fd, password, MAXLINE) != -1) {
...
static inline int act(int fd) {
    ...
    game = clients[fd].game;
    if (game->players[game->turn] != fd) {
        game_finish(game, game->players[game->turn]);
    }

    if (read_uint32_from_net(fd, &x) < 0 || read_uint32_from_net(fd, &y) < 0) {  // invalid step
        game_finish(game, game->players[!game->turn]);
        return 1;
    }
    game_act(game, x, y);
